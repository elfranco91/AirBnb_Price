...
class AirSpider(scrapy.Spider):
    ...
    def start_requests(self):
        global city
        yield SeleniumRequest(url = "https://www.airbnb.com/s/"+city+"/homes?refinement_paths%5B%5D=%2Ffor_you&search_type=search_query",
                              wait_time = 1,
                              callback = self.parse)
   ...
   #send the name of the city you inputted into the search box on the website
    def parse(self, response):
        ...
        for houses in house:
            ...
            ii = str(i)
            if link:
                yield{'link': 'https://airbnb.com'+link, 'name': name, 'rating':rating, 'guests': guests,
        ...
        if next_page:
            yield SeleniumRequest(url = 'https://www.airbnb.com'+next_page, callback = self.parse, wait_time =3)
